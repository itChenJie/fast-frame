...
public class SseEmitterServiceImpl implements SseEmitterService {
    ...
    private void sendMsgToClientByClientId(String clientId, MessageVo messageVo, SseEmitter sseEmitter) {
        ...
        log.info("sse send msg:{}",messageVo.toString());
        try {
    ...
    private Runnable completionCallBack(String clientId) {
        return () -> {
            log.info("sse end connection：{}", clientId);
            removeUser(clientId);
        };
    }
    ...
    private Consumer<Throwable> errorCallBack(String clientId) {
        return throwable -> {
            log.error("sse errorCallBack：连接异常,客户端ID:{}", clientId);
            ...
            for (int i = 0; i < 5; i++) {
                try {
                    ...
                    SseEmitter sseEmitter = sseCache.get(clientId);
                    if (sseEmitter == null) {
                        log.error("sse errorCallBack：第{}次消息重推失败,未获取到 {} 对应的长链接", i + 1, clientId);
                        continue;
                    }
    ...
    private void removeUser(String clientId) {
        ...
        log.info("sse removeUser：{}", clientId);
    }
}
